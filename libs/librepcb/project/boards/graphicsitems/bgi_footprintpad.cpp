...
void BGI_FootprintPad::updateCacheAndRepaint() noexcept
{
    ...
    mShape = mLibPad.getRegion().toQPainterPathPx();
    mStopMask = Path::toQPainterPathPx(mLibPad.getOutline().offset(stopMaskClearance));
    mCreamMask = Region::toQPainterPathPx(mLibPad.getRegion().offset(creamMaskClearance));
